...
export const recipeRoutes = (app, sql, upload, bucket) => {
    ...
    app.get("/recipes", async (req, res) => {
        ...
        const recipeWithImageURL = await Promise.all(
            recipes.map(async (rec) => {
                return {
                    ...rec,
                    image_url: await RecipeImage.get_image_url(
                        rec.gcs_file_name
                    ),
                };
            })
    ...
    app.get("/recipe-id/:id", async (req, res) => {
        ...
        const image_url = await file
            .getSignedUrl(imgConfig)
            .then(function (data) {
                return data[0];
            });
