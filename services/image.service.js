export default class ImageService {
    ...
    uploadImage(imageData) {
        ...
        return new Promise((resolve, reject) => {
            file.save(imageBuffer, options, function (err) {
                if (!err) {
                    console.log(
                        `File: ${file_name} has been uploaded to bucket`
                    );
                    resolve();
                } else {
    ...
    async checkImageExists(file_name, res) {
        ...
        try {
            ...
            if (exists) {
                console.log(
                    `File: '${file_name}' already exists. Image upload aborted.`
                );
                res.status(409).send({
                    error_message: `File: '${file_name}' already exists. Please rename image file and try again.`,
                });
                return true;
            } else {
                console.log(
                    `Upload permitted. File: '${file_name}' does not exist. `
                );
                return false;
            }
        } catch (error) {
            ...
            res.status(500).send({
                error_message:
                    "Error checking file existence. Image not uploaded.",
            });
            return true;
        }
    }
